...
- 最外层为 `NameSpace`，默认值为 `public`，可以在Nacos控制台中创建新的Namespace。
- 第二层为 `Group`，默认值为 `DEFAULT_GROUP`，可以在Nacos控制台中创建新的Group。
- 第三层为 `Service`，每个Service对应一个微服务，例如 `user-service`、`order-service`等。
- 第四层为 `Cluster`，默认值为 `DEFAULT`，可以在Nacos控制台中创建新的Cluster。
- 第五层为 `Instance`，每个Instance对应一个微服务的实例，例如 `user-service-1`、`user-service-2`等。
